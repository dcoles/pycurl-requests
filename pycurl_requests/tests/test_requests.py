...
def test_get_params(http_server):
    response = requests.get(
        http_server.base_url + "/params", params={"q": "foo", "r": "bar"}
    )
    response.raise_for_status()
...
def test_get_params_list(http_server, sequence_type):
    response = requests.get(
        http_server.base_url + "/params",
        params=sequence_type([("q", "foo"), ("r", "bar"), ("q", "baz")]),
    )
    response.raise_for_status()
...
def test_get_headers(http_server):
    response = requests.get(
        http_server.base_url + "/headers",
        headers={
            "Foo": "foo",
            "bat": None,
            "Bar": b"bar",
            "Baz": "\U0001f60a".encode(),
        },
    )
    response.raise_for_status()
    ...
    assert (
        headers["Baz"].encode("iso-8859-1") == "\U0001f60a".encode()
    )  # Yes, this is mojibake


def test_get_response_headers(http_server):
    response = requests.get(
        http_server.base_url + "/response/headers",
        params=[("Foo", "foo"), ("FooBar", "foo"), ("FooBar", "bar")],
    )
    response.raise_for_status()
...
def test_get_response_set_cookie(http_server):
    response = requests.get(
        ...
        params=[
            ...
            (
                "Set-Cookie",
                "TestCookie=%3A%202595; expires=Thu, 01-Jul-2021 07:56:03 GMT; Max-Age=3600",
            ),
        ],
    )
    response.raise_for_status()
    ...
    assert (
        ...
        == "test!, PHPSESSID=8536fl1c5igh89aqsjuf3l40jm; path=/, TestCookie=%3A%202595; expires=Thu, 01-Jul-2021 07:56:03 GMT; Max-Age=3600"
    )
...
def test_cookies(http_server, cookies_):
    ...
    assert response.text == "\n".join(
        "{}: {}".format(*item) for item in cookiejar.items()
    )
