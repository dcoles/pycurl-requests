...
if __name__ == "__main__":
    if not os.path.exists("Pipfile"):
        print(
            "ERROR: Requests' unit tests expect to run from the top-level of the repository",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "ERROR: Requests' unit tests require additional fixtures:", file=sys.stderr
        )
        print(
            "  pip install --user {}".format(" ".join(REQUIRED_PACKAGES)),
            file=sys.stderr,
        )
        sys.exit(1)
